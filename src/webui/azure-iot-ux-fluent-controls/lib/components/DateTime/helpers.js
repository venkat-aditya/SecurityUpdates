...
exports.placeholders = ["mm/dd/yyyy", "dd/mm/yyyy", "yyyy/mm/dd"];
const formatters = [
...
exports.formatDate = (date, format, localTimezone) => {
    const month = (localTimezone ? date.getMonth() : date.getUTCMonth()) + 1,
        monthString = month > 9 ? `${month}` : `0${month}`,
        day = localTimezone ? date.getDate() : date.getUTCDate(),
        dayString = day > 9 ? `${day}` : `0${day}`,
        year = localTimezone ? date.getFullYear() : date.getUTCFullYear();
    return formatters[format](`${year}`, monthString, dayString);
};
exports.replaceAt = (value, index, newValue) => {
    return (
        value.substr(0, index) + newValue + value.substr(index, +value.length)
    );
};
