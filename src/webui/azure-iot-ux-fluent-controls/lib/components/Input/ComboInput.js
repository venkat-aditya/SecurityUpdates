...
const React = require("react"),
    ...
    css = classNames.bind(require("./ComboInput.module.scss")),
    defaultMap = (option) => {
    ...
    defaultFilter = (newValue, option) => !option.hidden,
    defaultSelect = (newValue, option) => option === newValue,
    defaultLabel = (newValue, option) => option.label;
/**
...
class ComboInput extends React.Component {
    constructor(props) {
        ...
        this.optionFilter = props.optionFilter
            ? props.optionFilter
            : defaultFilter;
        this.optionSelect = props.optionSelect
            ? props.optionSelect
            ...
            switch (event.keyCode) {
                case Common_1.keyCode.down:
                    if (this.currentOption) {
                        const currentOption = options.indexOf(
                                this.currentOption
                            ),
                            index =
                                currentOption > -1
                ...
                case Common_1.keyCode.up:
                    if (this.currentOption) {
                        const currentOption = options.indexOf(
                            ...
                            index = currentOption > -1 ? currentOption : 0,
                            newValue =
                                index === 0
    ...
    getVisibleOptions(getDisabled = true) {
        ...
        return getDisabled
            ? results
            : results.filter((option) => !option.disabled);
    }
    onInputChange(event) {
        const newValue = event.target.value,
            options = this.getVisibleOptions(),
            result = options.filter((option) =>
                this.optionSelect(newValue, option)
            );
        if (result.length > 0) {
    ...
    clearSelection(option) {
        ...
        if (option && this.optionElements[option]) {
            const element = this.optionElements[option],
                options = this.getVisibleOptions(),
                index = options.map((option) => option.value).indexOf(option),
                className =
                    this.props.attr.option && this.props.attr.option.className
    ...
    render() {
        ...
        const visibleOptions = this.getVisibleOptions(),
            options = visibleOptions.map((option, index) => {
                const checkLabel = this.props.showLabel
                    ? this.props.optionLabel(value, option).toString ===
                      this.props.value
                    : false;
                ...
                return React.createElement(
                    Attributes_1.Elements.button,
                    {
                        ...
                        attr: Attributes_1.mergeAttributes(
                            this.props.attr.option,
                            option.attr
                        ),
                    },
        ...
        const clearButton =
            ...
            validOptions = visibleOptions.filter((option) => !option.disabled);
        return React.createElement(
            Dropdown_1.Dropdown,
            {
                dropdown: dropdown,
